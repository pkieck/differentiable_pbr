...
it drives ∇·u = 0 (volumetric, see Test 6).

Here the density is a genuine field set by the ideal-gas equation of state at a
*fixed thermodynamic pressure* p₀ (low-Mach / "isobaric" approximation, standard
for reactive packed-bed flows):

      ρ(r,z) = p₀ · M̄ / (R · T_f),     M̄ = 1 / Σ_k (Y_k / M_k)
...
constraint is now on the **mass flux**:
      ...
      ∂p′/∂t = −β² · ε · ∇·(ρ u)        ⇒  steady state:  ∇·(ρ u) = 0

(not ∇·u = 0).  Consequently the *superficial mass flux* ρu is divergence-free,
and where the gas heats up the velocity must rise to keep ρu·A constant.

Scalar (energy, species) advection uses the **mass flux** ρu at the faces, with
the same uniform-field-annihilating correction the incompressible solver uses:

      ∇·(ρ u q) − q_c · ∇·(ρ u)   →   ρ u · ∇q     (non-conservative form)

This annihilates a uniform field even when ∇·(ρu) ≠ 0 (under-converged /
pseudo-transient state), exactly as the constant-ρ solver subtracts q_c·div(u).

When T and composition are uniform, ρ is constant and every term reduces
...
struct CompressibleParams{T, PM, G<:IdealGas, S<:SolidProps, RC, BC<:BoundaryConditions}
    ...
    gravity::T
    D_species::Vector{T}
    M_species::Vector{T}   # molar masses [kg/mol] for EOS M̄ (length Ns)
    bcs::BC
end
...
"""
    ideal_gas_density(p0, Mbar, T) → ρ  [kg/m³]

ρ = p₀ · M̄ / (R · T).  `R_GAS` is defined in closures.jl.
"""
...
function rhs_compressible!(du, u, p::CompressibleParams, t)
    ...
    for j in 1:nz, i in 1:nr
        ...
        # Neumann at axis & walls, Dirichlet (= ρ_in) at the inlet.
        ...
        # ── Mass flux ρu at each face [kg/(m²·s)] ─────────────────────────
        ...
        # div(ρu) in axisymmetric FVM:
        div_mass = ( rf_p * G_r_p - rf_m * G_r_m ) / Ar +
                   ( G_z_p - G_z_m ) / dz

        # ε kept as a per-cell scalar (parity with incompressible solver and
        # Brinkman masking); steady state ⇒ div(ρu) = 0 regardless of ε.
        ...
        dur[i,j] = eps / rho_c * (-dp_dr + μ_eff * visc_r - D_drag * ur_c)
        ...
        duz[i,j] = eps / rho_c * (-dp_dz + μ_eff * visc_z - D_drag * uz_c - rho_c * grav)
        ...
        for k in 1:Ns
            ...
            # Diffusive flux uses the face density: ∇·(ρ D ∇Y)
